...
class RamblerCookiesParser:
        ...
        try:
            for p in self.logs_dir.rglob("*.txt"):
                ...
                if self._is_netscape_file(p):
                    files.append(p)
            self.logger.info(f"Найдено кандидатов на куки Rambler: {len(files)}")
            return files
    ...
    def _belongs_to_rambler(self, cookies: List[Dict]) -> bool:
        domain_hits = 0
        name_hits   = 0
        for c in cookies:
            dom = c["domain"].lower()
            name = c["name"]
            if any(d in dom for d in ["rambler.ru","lenta.ru"]):
                domain_hits += 1
            if name in self.cookie_names:
                name_hits += 1
        return domain_hits >= 2 or (domain_hits >= 1 and name_hits >= 1)

    def get_all_rambler_cookies_files(self) -> List[str]:
        candidates = self.find_cookies_files()
        scored = []
        for p in candidates:
            cookies = self.parse_file(p)
            if self._belongs_to_rambler(cookies):
                score = self._score(cookies)
                scored.append((str(p), score))

        scored.sort(key=lambda x: x[1], reverse=True)
        files = [p for p, _ in scored]
        self.logger.info(f"Найдено валидных файлов Rambler: {len(files)}")
        return files

    def _score(self, cookies: List[Dict]) -> int:
        score = 0
        important = {"rambler_id", "rur", "top100_id"}
        for c in cookies:
            if c["name"] in important:
                score += 15
            if c["name"] in self.cookie_names:
                score += 5
            if any(d in c["domain"] for d in [".rambler.ru","mail.rambler.ru"]):
                score += 3
        return score

def get_all_rambler_cookies_files(logs_dir: str = "logs") -> List[str]:
    return RamblerCookiesParser(logs_dir).get_all_rambler_cookies_files()